<script>
  ...
  let showGallery = true; // New state to prevent Hero flash
  ...
  const getTransitionName = (title) => `title-${title.toLowerCase().replace(/\s+/g, '-')}`;
  ...
  async function openProject(event) {
    ...
    showGallery = false; // Hide gallery/hero immediately
  ...
  function close() {
    isTransitioning = true;
    // We keep selectedProject active during the 'out' transition
    
    setTimeout(() => {
      selectedProject = null;
      showGallery = true; // Only show gallery once the transition is complete
      window.scrollTo({ top: 0, behavior: 'instant' });
      setTimeout(() => {
        isTransitioning = false;
      }, 50);
...
<div class="relative w-full min-h-[80vh]">
  ...
  {#if !selectedProject && showGallery}
    <div 
      in:fade|local={{ duration: 400, delay: 100 }} 
  ...
  {:else if selectedProject}
    ...
    >
      ...
      <header class="mb-20">
        <h2 
          style="view-transition-name: {getTransitionName(selectedProject.title)}"
          class="text-6xl md:text-9xl font-black text-slate-900 dark:text-white mb-8 tracking-tighter leading-[0.85]"
        >
          {selectedProject.title}
        </h2>
        <p class="text-2xl md:text-4xl font-bold text-slate-500 dark:text-slate-600 text-left">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
        <div class="lg:col-span-8 space-y-12">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16">
            <div class="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-800">
              <h4 class="font-black uppercase tracking-widest text-[10px] text-slate-500 dark:text-slate-400 mb-4 text-left">Architecture</h4>
              <p class="text-slate-900 dark:text-white font-bold text-left">Built for scale with {selectedProject.tech[0]} and {selectedProject.tech[1] || 'modern patterns'}.</p>
            </div>
            <div class="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-800">
          ...
          {#if selectedProject.metricsImage}
            <div class="mt-16 space-y-6">
              <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400 text-left">Vitals & Core Performance</h4>
              <button 
              ...
              >
                <img src={selectedProject.metricsImage} alt="Score" class="w-full h-auto transition-all duration-700 group-hover:scale-[1.02]" />
                <div class="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/10 transition-colors flex items-center justify-center">
                  <span class="opacity-0 group-hover:opacity-100 bg-white/95 dark:bg-slate-900/95 text-slate-900 dark:text-white px-4 py-2 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all shadow-lg">Expand Metrics</span>
                </div>
              </button>
            </div>
          {/if}
        ...
        <aside class="lg:col-span-4 lg:sticky lg:top-24 h-fit space-y-12 text-left">
          <div class="space-y-6">
            ...
            <div class="flex flex-wrap gap-2">
              {#each selectedProject.tech as tag}
                <span class="px-3 py-1 text-[10px] font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded border border-slate-200 dark:border-slate-700">{tag}</span>
              {/each}
            </div>
          </div>
          <div class="pt-8 border-t border-slate-100 dark:border-slate-800 space-y-4">
            <a href={selectedProject.projectUrl} target="_blank" rel="noopener noreferrer" class="flex items-center justify-between w-full px-8 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-black text-sm uppercase tracking-widest hover:bg-slate-700 dark:hover:bg-slate-200 transition-all">Launch Project <span class="text-xl">↗</span></a>
            {#if selectedProject.github}
              <a href={selectedProject.github} target="_blank" rel="noopener noreferrer" class="flex items-center justify-between w-full px-8 py-5 border-2 border-slate-900 dark:border-white text-slate-900 dark:text-white font-black text-sm uppercase tracking-widest hover:bg-slate-900 hover:text-white dark:hover:bg-white dark:hover:text-slate-950 transition-all">Source Code <span class="text-xl">⌘</span></a>
            {/if}
          </div>
        </aside>
      </div>
  ...
  {#if showMetrics && selectedProject}
    <div class="fixed inset-0 z-[1000] flex items-center justify-center p-6 md:p-24" transition:fade={{ duration: 200 }}>
      <button type="button" class="absolute inset-0 bg-slate-950/70 backdrop-blur-md cursor-zoom-out w-full h-full border-none outline-none" on:click={toggleMetrics} aria-label="Close"></button>
      <div class="relative max-w-5xl w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10" transition:scale={{ start: 0.98, duration: 300, easing: expoOut }}>
        <img src={selectedProject.metricsImage} alt="Metrics" class="w-full h-auto block" />
      </div>
    </div>
  {/if}
</div>